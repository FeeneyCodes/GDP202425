...
public:
	...
	glm::vec4 getColourRGBA( CGLColourHelper::enumColours colourEnum );
	glm::vec4 getColourRGBA( std::string colourName );
	...
	glm::vec4 getRandomColourRGBA(void);
	CGLColourHelper::enumColours getRandomColourEnum(void);
	void ShuffleRandomColours(void);

	// Complimentary colours, etc.
	glm::vec3 getHSLfromRGB(glm::vec3 RGB);
	// The alpha value is ignored
	glm::vec4 getHSLfromRGBA(glm::vec4 RGBA);
	glm::vec3 getRBGfromHSL(glm::vec3 HSL);
	// The alpha value is ignored
	glm::vec4 getRBGAfromHSL(glm::vec3 HSL);
	glm::vec3 getComplimentaryRGB(glm::vec3 RGB);	
	// The alpha value is ignored
	glm::vec4 getComplimentaryRGBA(glm::vec4 RGBA);

	float getRGBfromHue( float v1, float v2, float vH );
	...
	float floatEpsilonCompare;// = DEFAULT_EPSION_COMPARE
private:
	...
	float floatIsEqual( float x, float y )
	{
		float epsilon = floatEpsilonCompare * fmax(1.0f, fmax( fabs(x), fabs(y) ) );
		if ( fabs(x - y) <= epsilon )
		{
			return true;
		}
	...
	float floatIsZero( float x )
	{
		if ( this->floatIsEqual( x, this->floatEpsilonCompare ) )
		{
			return true;
		}
		return false;
