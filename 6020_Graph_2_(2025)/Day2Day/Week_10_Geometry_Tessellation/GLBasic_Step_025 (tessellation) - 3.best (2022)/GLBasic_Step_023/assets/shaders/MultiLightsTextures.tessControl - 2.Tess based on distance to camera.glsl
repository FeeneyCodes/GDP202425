...
//  Sexy code we will use in a moment
...
//	}
	...
	// If we are at a certain distance (close), tessellate, otherwise don'tessDistance
	if ( length(vecEyeVert) < tessDistance )
	{	// "close" to camera, so tesselate
