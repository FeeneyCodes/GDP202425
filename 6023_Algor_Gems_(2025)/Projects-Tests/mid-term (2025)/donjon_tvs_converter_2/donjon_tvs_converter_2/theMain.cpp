...
int main(int argc, char* argv[])
{
	if (argc != 2)
	{
		std::cout << "Usage: donjon_tsv_converter_2  tsvFile.tsv" << std::endl;
...
//	std::vector< std::vector< std::string > > vec2DMaze;
	...
	std::cout << "Placing food and water...";

	unsigned int numRows = (unsigned int)vec2DMaze.size();
	unsigned int numCols = (unsigned int)vec2DMaze[0].size();

	unsigned int waterPlaced = 0;
	while ( waterPlaced < 100 )
	{
		unsigned int theRow = rand() % numRows;
		unsigned int theCol = rand() % numCols;
		if ( vec2DMaze[theRow][theCol] == "F" )
		{
			vec2DMaze[theRow][theCol] = "1";
			waterPlaced++;
		}
	}//while ( waterPlaced < 100 )

	unsigned int foodPlaced = 0;
	while ( foodPlaced < 50 )
	{
		unsigned int theRow = rand() % numRows;
		unsigned int theCol = rand() % numCols;
	...
	for ( unsigned int row = 0; row != vec2DMaze.size(); row++ )
	{
		for ( unsigned int col = 0; col != vec2DMaze[row].size(); col++ )
		{
			...
			else if ( curToken == "1" )	// Food
			{
				ssOutput << "f";
			}
			else if ( curToken == "2" ) // Water
			{
				ssOutput << "w";
			}
			else
