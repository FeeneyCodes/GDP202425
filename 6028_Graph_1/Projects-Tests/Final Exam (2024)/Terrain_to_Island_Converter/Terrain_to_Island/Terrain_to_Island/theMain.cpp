...
int main(int argc, char* argv[])
{
	...
	// The assumption is Y is height, so in MeshLab, so rotate by -90 degrees along the x axis.
	// Scale by 400x 
	glm::mat3 matRotate = glm::rotate(glm::mat4(1.0f), glm::radians(-90.0f), glm::vec3(1.0f, 0.0f, 0.0f));
	for (sVertex& curVert : vecVerts)
	{
		glm::vec3 rotatedVert = matRotate * glm::vec3(curVert.pos.x, curVert.pos.y, curVert.pos.z);
		curVert.pos.x = rotatedVert.x;
		curVert.pos.y = rotatedVert.y;
		curVert.pos.z = rotatedVert.z; 
	}
