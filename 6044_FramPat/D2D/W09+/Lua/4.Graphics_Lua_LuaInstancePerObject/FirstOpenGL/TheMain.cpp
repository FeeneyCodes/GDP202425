...
void RunLuaScriptOnObjects(float deltaTime);
...
int main(void)
{
	// TODO: Fix the bugs, yo
//	std::cout
//		<< "There's an exception in the static vector with Lua 5.4.7 "
...
//	::g_pTheMeshTypeManager->LoadPlyFileIntoGLBuffer( shadProgID, "dragon_vrip_res2_N.ply" );
	...
	if ( ! ::g_pTextureManager->Create2DTextureFromBMPFile( "trudeau_transparency_20140611.bmp", true ) )
	{
...
//	cLuaBrain* pABrain = new cLuaBrain();
//	std::string somescript =
//		"a = {} \n" \
//		"a[0] = 1.1 \n " \
//		"a[1] = 2.2 \n " \
//		"a[2] = 3.3 \n " \
//		"CreateCommand('MoveCommand', a )";
////	pABrain->RunScriptImmediately("CreateCommand('MoveCommand', 5.4, 2.3, 1.2)");
//	pABrain->RunScriptImmediately(somescript.c_str());
...
//
//	::p_LuaScripts->LoadScript( "MoveTo", ssMoveTo.str() );
...
void RunLuaScriptOnObjects(float deltaTime)
{
	for ( int index = 0; index != ::g_vec_pGOs.size(); index++)
	{
		// Does this object have a Lua "brain" object?
		if ( ::g_vec_pGOs[index]->pLuaBrain )
		{	// Yes, so run all script inside 
			::g_vec_pGOs[index]->pLuaBrain->Update( deltaTime );
		}
	}
